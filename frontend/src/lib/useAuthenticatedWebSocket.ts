...
export function useAuthenticatedWebSocket({ onMessage, onConnectionChange }: WebSocketClientProps = {}) {
    const auth = useAuth();
    ...
    const connect = async () => {
        ...
        try {
            ...
            const ws = new WebSocket(`${wsUrl}/ws?token=${auth.user.access_token}`);
