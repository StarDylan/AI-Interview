...
class WebRTCConnectionHandler:
    ...
    def __init__(self, websocket, transcription_manager: GlobalTranscriptionManager):
        ...
        logger.info(
            f"New WebRTC connection handler created for {websocket.remote_address}"
        )
    ...
    async def initialize_session(self):
        ...
        self.transcription_service = self.transcription_manager.create_session_service(
            self.session
        )

        logger.info(
            f"Initialized session {self.session.session_id[:8]} for connection {self.websocket.remote_address}"
        )
    ...
    async def _handle_offer(self, data):
        ...
        async def on_track(track):
            if not self.session:
                ...
                return
            logger.info(
                f"Session {self.session.session_id[:8]}: Received track: {track.kind}"
            )
    ...
    async def _handle_ice_candidate(self, data):
        ...
        if self.peer_connection and self.session:
            try:
                ...
                logger.debug(
                    f"Added ICE candidate for session {self.session.session_id[:8]}"
                )
            except Exception as e:
    ...
    async def _process_audio_track(self, track):
        ...
        if (
            ...
            or not self.transcription_service
            or not self.session
        ):
        ...
        try:
            while True:
                ...
                if self.audio_processor and self.transcription_service:
                    ...
                    if len(audio_array) > 0:
                        ...
                        transcription = self.transcription_service.process_audio_chunk(
                            audio_bytes
                        )
            ...
            logger.error(
                f"Error processing audio track for session {self.session.session_id[:8] if self.session else 'unknown'}: {e}"
            )
        ...
        try:
            message = {
                ...
                "data": {
                    ...
                    if self.session.transcription_buffer
                    else None,
                },
            }
            await self.websocket.send(json.dumps(message))
            logger.debug(
                f"Sent transcription update for session {self.session.session_id[:8]}"
            )
    ...
    async def _finalize_session(self):
        ...
        if self.session and self.session.is_active:
            ...
            if self.audio_processor:
                ...
                if audio_file:
                    logger.info(
                        f"Session {self.session.session_id[:8]} audio saved to: {audio_file}"
                    )

            # Finalize transcription
            final_transcription = self.transcription_manager.finalize_session(
                self.session.session_id
            )
            if final_transcription:
                logger.info(
                    f"Session {self.session.session_id[:8]} final transcription: {final_transcription}"
                )
    ...
    async def _send_final_transcription(self, final_text: str):
        ...
        try:
            ...
            await self.websocket.send(json.dumps(message))
            logger.info(
                f"Sent final transcription for session {self.session.session_id[:8]}"
            )
    ...
    async def cleanup(self):
        ...
        logger.info(
            f"Cleaned up WebRTC connection handler for session {self.session.session_id[:8] if self.session else 'unknown'}"
        )
...
class WebRTCServer:
    ...
    async def handle_client(self, websocket):
        ...
        logger.info(
            f"New client connected: {websocket.remote_address}, Session: {handler.session.session_id[:8] if handler.session else 'unknown'}"
        )
        ...
        try:
            ...
            logger.info(
                f"Connection handler removed for session {handler.session.session_id[:8] if handler.session else 'unknown'}"
            )
