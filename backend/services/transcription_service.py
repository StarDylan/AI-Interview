...
class SessionTranscriptionService:
    ...
    def __init__(self, session: TranscriptionSession):
        ...
        logger.info(
            f"Transcription service initialized for session {self.session.session_id[:8]}"
        )
    ...
    def _initialize_vosk(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Vosk model for session {self.session.session_id[:8]}: {e}"
            )
            raise
    ...
    def process_audio_chunk(self, audio_chunk: bytes) -> Optional[str]:
        ...
        if not self.session.is_active:
            logger.warning(
                f"Attempted to process audio for inactive session {self.session.session_id[:8]}"
            )
            return None

        if not self.recognizer:
            logger.error(
                f"Recognizer not initialized for session {self.session.session_id[:8]}"
            )
            return None
        ...
        try:
            ...
            if self.recognizer.AcceptWaveform(audio_chunk):
                ...
                if text:
                    ...
                    logger.info(
                        f"Session {self.session.session_id[:8]} transcription: {text}"
                    )
                    return text
            ...
            if partial_text:
                logger.debug(
                    f"Session {self.session.session_id[:8]} partial: {partial_text}"
                )
            ...
            logger.error(
                f"Error processing audio chunk for session {self.session.session_id[:8]}: {e}"
            )
            return None
    ...
    def finalize_transcription(self) -> Optional[str]:
        ...
        try:
            ...
            if final_text:
                ...
                logger.info(
                    f"Session {self.session.session_id[:8]} final transcription: {final_text}"
                )
            ...
            logger.error(
                f"Error finalizing transcription for session {self.session.session_id[:8]}: {e}"
            )
            return None
    ...
    def _save_transcription_file(self) -> Optional[Path]:
        ...
        try:
            ...
            text_filename = (
                f"transcription_{timestamp}_{self.session.session_id[:8]}.txt"
            )
            ...
            logger.info(
                f"Session {self.session.session_id[:8]} transcription saved to: {filepath}"
            )
            ...
            logger.error(
                f"Error saving transcription file for session {self.session.session_id[:8]}: {e}"
            )
            return None
...
class GlobalTranscriptionManager:
    ...
    def create_session_service(
        ...
        logger.info(
            f"Created transcription service for session {session.session_id[:8]}"
        )
        return service

    def get_session_service(
        self, session_id: str
    ) -> Optional[SessionTranscriptionService]:
    ...
    def cleanup_inactive_sessions(self):
        ...
        inactive_sessions = [
            session_id
            for session_id, service in self.active_sessions.items()
